...
class FirstComponentTest : ComponentTest by ComponentTestPreparation() {
    ...
    fun `when fetchDataUseCase fails, viewState contains error`() = runComponentTest {
        val component = createComponent(
            fetchDataUseCase = { throw NetworkError.ConnectionError(IOException()) },
            counterUseCase = { flowOf() },
        ...
        turbineScope {
            ...
            state.skipItems(1) // Skip default state
            assertIs<NetworkError.ConnectionError>(state.awaitItem().randomPersonError)
        }
    }
    ...
    fun `counter updates viewState correctly with each emission`() = runComponentTest {
        val component = createComponent(
            ...
            counterUseCase = {
                flow {
                    // Start from 1 to avoid StateFlow deduplication with the initial counter=0
                    for (i in 1..6L) {
                        emit(i)
                        delay(it.interval)
                    }
        ...
        turbineScope {
            ...
            lifecycleRegistry.create()
            states.skipItems(1) // Skip initial state (counter=0)
            ...
            assertEquals(6L, states.awaitItem().counter)
        }
    }
